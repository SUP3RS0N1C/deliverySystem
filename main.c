...
{	//유효하지 않은 입력이 들어오면 -1을 반환, 다시 입력받도록 함  
	if (nBuilding <= 0 || nBuilding > N_BUILDING)//음수를입력or존재하는 빌딩 수 이상의 수 입력   
	{
		return -1;
	}
	
	if (nRoom/100 <= 0 || nRoom/100 > N_FLOOR)//음수입력or존재하는 층 수 이상의 수 입력  
	{
		return -1;
	}
	
	if (nRoom%100 <= 0 || nRoom%100 > N_ROOM)//음수입력or존재하는 방 수 이상의 수 입력  
	{
		return -1;
	}
