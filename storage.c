...
int str_createSystem(char* filepath) {
	...
	fscanf(fp, "%s", &masterPassword[PASSWD_LEN+1]);//마스터키 받기
	...
	{
		fscanf(fp, "%i %i %d %d %s %s", &x, &y, &deliverySystem[x][y].building, &deliverySystem[x][y].room, deliverySystem[x][y].passwd, &deliverySystem[x][y].context);//초기 저장된 택배 정보 받기  
	}
...
int str_extractStorage(int x, int y) {
	...
	if()//입력한 비번이 맞으면 내용을 출력
	{
		//printStorageInside(x,y); 
		//deliverySystem[x][y].cnt=0 해서 저장안되어있음을 알려야함 
	}
